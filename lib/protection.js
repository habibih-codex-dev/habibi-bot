...
async function runProtections(p) {
  ...
  // ===================== ANTI-HIDETAG =====================
  // Member biasa pakai trik hidetag (mention banyak orang tanpa teks @).
  // Deteksi: jumlah mentionedJid banyak TANPA menampilkan "@" di teks.
  if (g.antihidetag) {
    try {
      const mentioned = msg.message?.extendedTextMessage?.contextInfo?.mentionedJid || [];
      const visibleTags = (body.match(/@/g) || []).length;
      if (mentioned.length >= 5 && visibleTags < mentioned.length) {
        await conn.sendMessage(from, {
          text: `🛡️ *ANTI-HIDETAG*\n@${senderNumber} terdeteksi memakai tag tersembunyi. Pesan dihapus.`,
          mentions: [sender],
        });
        await del();
        return true;
      ...
      console.error('[PROT] error antihidetag:', e.message);
    }
  }

  // ===================== ANTILINKWA V2 (SISTEM WARNING) =====================
  // Setiap kirim link grup WA -> hapus + warn++. Jika warn >= maxWarn -> kick + reset.
  if (g.antilinkwav2 && RE_WA_GROUP.test(body)) {
    try {
      await del();
      const maxWarn = Number(config.maxWarn) || 3;
      const current = db.addWarn(sender, 1);

      if (current >= maxWarn) {
        await conn.sendMessage(from, {
          text: `🚫 @${senderNumber} telah mencapai batas pelanggaran (${current}/${maxWarn}) dan dikeluarkan dari grup!`,
          mentions: [sender],
        });
        await kick();
        db.resetWarn(sender); // reset poin setelah di-kick
      } else {
        await conn.sendMessage(from, {
          text: `⚠️ @${senderNumber} Jangan share link grup! Peringatan kamu: [${current}/${maxWarn}]. Jika mencapai ${maxWarn}x, kamu akan di-kick otomatis!`,
          mentions: [sender],
        });
      }
    } catch (e) {
      console.error('[PROT] error antilinkwav2:', e.message);
    }
    return true;
  }
  ...
  const rules = [
    ...
    },
    {
      ...
      kick: false, // V1: hanya delete (gunakan antilinkwav2 untuk kick)
    },
    {
      on: g.antilinkv2,
